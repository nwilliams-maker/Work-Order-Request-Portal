...
st.markdown(f"""
    ...
    div[data-testid="stIFrame"] {{ resize: vertical !important; overflow: auto !important; padding-bottom: 20px; background: white; border-radius: 8px; border: 1px solid #d0d4e4; }}
    .route-counter {{ font-size: 18px; font-weight: 600; color: {TB_PURPLE}; padding: 5px 0px; }}
    ...
    div[data-testid="stExpander"] details[open] summary {{ border-bottom: 1px solid #d0d4e4 !important; }}
    div[data-testid="stExpander"] details summary p {{ color: #323338 !important; font-weight: 600 !important; font-size: 16px !important; }}
    div[data-testid="stExpander"] details summary svg {{ fill: #323338 !important; color: #323338 !important; }}
...
def run_pod_tab(pod_name):
    ...
    c1.markdown(
        f"<div class='metric-box'><div class='metric-title'>Total Routes</div><div class='metric-value'>{len(clusters)}</div></div>",
        unsafe_allow_html=True)
    c2.markdown(
        f"<div class='metric-box'><div class='metric-title' style='color:{TB_GREEN};'>Ready to Dispatch</div><div class='metric-value'>{len(acc)}</div></div>",
        unsafe_allow_html=True)
    c3.markdown(
        f"<div class='metric-box'><div class='metric-title' style='color:#f44336;'>Manual Review</div><div class='metric-value'>{len(unacc)}</div></div>",
        unsafe_allow_html=True)
    with c4:
...
def run_global_tab():
    ...
    for idx, (name, cfg) in enumerate(POD_CONFIGS.items()):
        ...
        m_cols[idx].markdown(
            f"<div class='metric-box'><div class='metric-title'>{name}</div><div class='metric-value'>{cnt} Tasks</div></div>",
            unsafe_allow_html=True)
