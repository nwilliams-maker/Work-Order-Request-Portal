...
def process_pod_data(pod_name, p_bar=None, progress_step=0.0):
    ...
    while True:
        ...
        batch = res.json().get('tasks', [])
        all_tasks.extend(batch)
        if res.json().get('lastId') and len(batch) > 0:
            url = f"https://onfleet.com/api/v2/tasks/all?state=0&from={int(time.time() * 1000) - (80 * 24 * 3600 * 1000)}&lastId={res.json()['lastId']}"
        else: break
