...
st.markdown(f"""
    <style>
    .stApp {{ background-color: #f4f5f7 !important; color: #323338 !important; font-family: 'Inter', sans-serif; }}
    h1, h2, h3 {{ color: {TB_PURPLE} !important; font-weight: 700 !important; }}
    #status {{ display: none !important; }}
    [data-testid="stStatusWidget"] {{ display: none !important; }}
    .stTabs [data-baseweb="tab-list"] {{ gap: 24px; border-bottom: 1px solid #d0d4e4; }}
    .stTabs [data-baseweb="tab"] {{ height: 50px; white-space: pre-wrap; background-color: transparent; padding: 10px 16px; font-size: 16px; font-weight: 500; color: #676879; }}
    ...
    .metric-title {{ font-size: 11px; text-transform: uppercase; color: #676879; font-weight: 600; }}
    .metric-value {{ font-size: 20px; color: {TB_PURPLE}; font-weight: 700; }}
    .stButton>button {{ background-color: {TB_PURPLE} !important; color: white !important; border: none !important; border-radius: 4px; width: 100%; font-weight: 600; padding: 10px; opacity: 1 !important; }}
    ...
    div[data-testid="stProgress"] > div > div {{ background-color: {TB_LIGHT_BLUE} !important; }}
    div[data-testid="stProgress"] > div > div > div {{ background-color: {TB_GREEN} !important; }}
    .stTextInput input, .stNumberInput input, .stDateInput input, .stTextArea textarea {{ background-color: white !important; color: #323338 !important; border: 1px solid #d0d4e4 !important; }}
    label p {{ color: #323338 !important; font-weight: 600 !important; }}
    </style>
...
def get_metrics(home, cluster_nodes, stop_rate):
    ...
    pay = max_cap if pre_pay > max_cap else pre_pay
    p_type = f"Capped at ${MAX_RATE_PER_STOP}/Stop" if pre_pay > max_cap else ("Hourly Minimum" if (hrs * HOURLY_FLOOR_RATE) > (stop_count * stop_rate) else "Per Stop Rate")
...
def process_pod_data(pod_name):
    ...
    with ui_container.container():
        ...
        while pool:
            ...
            for t in pool:
                if haversine(anchor['lat'], anchor['lon'], t['lat'], t['lon']) <= 30.0 and (len(unique_locs) < 20 or t['full_addr'] in unique_locs):
                    group.append(t); unique_locs.add(t['full_addr'])
                else: rem.append(t)
            pool = rem
            clusters.append({"data": group, "center": [anchor['lat'], anchor['lon']], "unique_count": len(unique_locs), "acceptable": len(unique_locs) >= 3})
...
def render_dispatch_logic(i, cluster, pod_name):
    ...
    rate = col_rate.number_input("Rate/Stop", 16.0, 22.0, suggested_rate, 0.5, key=f"rate_{i}_{pod_name}")
        ...
        if real_gas_id:
            ...
            st.markdown(f"""
                <a href="{mailto_link}" target="_blank" style="text-decoration: none;">
                    <div style="background-color: #76bc21; color: white; padding: 10px; text-align: center; border-radius: 4px; font-weight: bold; cursor: pointer; border: 1px solid #5e961a;">
            ...
            st.markdown("""
                <div style="background-color: #e2e8f0; color: #94a3b8; padding: 10px; text-align: center; border-radius: 4px; font-weight: bold; cursor: not-allowed; border: 1px solid #cbd5e1;">
...
def run_pod_tab(pod_name):
    ...
    if f"clusters_{pod_name}" not in st.session_state:
        st.info(f"Data for {pod_name} is resting.")
    ...
    c1.metric("Total", len(clusters))
    c2.metric("Ready", len(acc))
    c3.metric("Review", len(unacc))
    if c4.button("🔄 Refresh", key=f"refresh_{pod_name}"): process_pod_data(pod_name); st.rerun()
    m = folium.Map(location=clusters[0]['center'], zoom_start=6, tiles="cartodbpositron")
    for idx, c in enumerate(clusters):
        folium.CircleMarker(c['center'], radius=12, color=TB_GREEN if c in acc else "#f44336", fill=True, opacity=0.8).add_to(m)
    ...
    with t1:
        for i, cluster in enumerate(acc):
            with st.expander(f"📍 {cluster['data'][0]['city']}, {cluster['data'][0]['state']} | {cluster['unique_count']} Stops"): render_dispatch_logic(i, cluster, pod_name)
    with t2:
...
def run_global_tab():
    ...
    if "global_data" not in st.session_state:
        if st.button("🌍 Sync Global Overview Map", key="load_global"):
            ...
            st.session_state.global_data = [{"lat": t.get('destination', {}).get('location', [0, 0])[1], "lon": t.get('destination', {}).get('location', [0, 0])[0]} for t in all_tasks]
            st.rerun()
        return
    ...
    for t in st.session_state.global_data:
